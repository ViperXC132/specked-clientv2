...
public class BiomeHudFeature extends Feature {
    public BiomeHudFeature() { super("Biome Display", "Show current biome", FeatureCategory.HUD); }
    ...
    @Override public void onRender(DrawContext context, float tickDelta) {
        ...
        String biome = mc.world.getBiome(mc.player.getBlockPos()).getKey().map(key -> key.getValue().toString()).orElse("unknown");
