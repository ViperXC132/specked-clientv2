...
public class AccountsScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float partialTick) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, "Account Manager", this.width / 2, 20, 0xFFFFFF);
        int y = 50;
        for (Account account : SpekedClient.getInstance().getAccountManager().getAccounts()) {
            String active = account.isActive() ? " [ACTIVE]" : "";
            context.drawTextWithShadow(this.textRenderer, account.getUsername() + " (" + account.getType().getDisplayName() + ")" + active, 20, y, 0xFFFFFF);
            y += 12;
        }
    }
