...
public class PauseServerHostingScreen extends Screen {
    ...
    public PauseServerHostingScreen(Screen parent) { super(Text.literal("Server Hosting")); this.parent = parent; }
    @Override public void render(DrawContext context, int mouseX, int mouseY, float partialTick) {
        ...
        for (ServerHostingManager.ManagedServer server : manager.getServers()) {
            ...
            int color = switch (server.getStatus()) { case ONLINE -> 0xFF00FF00; case OFFLINE, CRASHED -> 0xFFFF0000; case STARTING, STOPPING -> 0xFFFFFF00; };
            context.drawTextWithShadow(this.textRenderer, "Status: " + server.getStatus().getDisplayName(), 20, y + 10, color);
            y += 25;
        }
    }
