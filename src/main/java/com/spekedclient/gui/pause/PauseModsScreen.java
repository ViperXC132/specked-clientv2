...
public class PauseModsScreen extends Screen {
    ...
    public PauseModsScreen(Screen parent) { super(Text.literal("Modules")); this.parent = parent; }
    @Override public void render(DrawContext context, int mouseX, int mouseY, float partialTick) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, "Enabled Modules", this.width / 2, 20, 0xFFFFFF);
        int y = 50;
        for (Feature feature : SpekedClient.getInstance().getFeatureManager().getFeatures()) if (feature.isEnabled()) {
            context.drawTextWithShadow(this.textRenderer, "• " + feature.getName() + " [ON]", 20, y, 0xFF00FF00);
            y += 12;
        }
    }
